...
extern struct event_history ev_history;

extern int mode;	// 0 for record mode, 1 for replay mode

void init_rec_event_struct(vr_log_t *ev);
void record_event_id(vr_log_t *ev, int int_type);
void vr_set_ic_irq(void);

extern unsigned int vk_hwi_count;
extern unsigned int vr_ic_int_cnt;
extern unsigned int vr_reg_fail_cnt[15];
extern unsigned int vr_bkpt_expt_cnt;
extern unsigned int *vr_data_ptr;

extern vr_log_t *current_log;		// current log structure pointer
extern unsigned int vr_replay_count;	// instruction counter for reproducing
extern unsigned int vr_fail_cnt;

extern unsigned char vr_replay_end_flag;

extern unsigned int race_var;
